			...
			163: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
				...
				class Strings {
					static init() {
						...
						modules_namespaceObject.Dispatcher.subscribe(constants_namespaceObject.ActionTypes.USER_SETTINGS_UPDATE, _classStaticPrivateFieldSpecGet(this, Strings, _handleLocaleChange));
					}
					static shutdown() {
						modules_namespaceObject.Dispatcher.unsubscribe(constants_namespaceObject.ActionTypes.USER_SETTINGS_UPDATE, _classStaticPrivateFieldSpecGet(this, Strings, _handleLocaleChange));
					}
				...
				const getLastChannelMessageDate = function(channelId) {
					return extractDate(stores_namespaceObject.Messages.getMessages(channelId)._array.slice(-1)[0]?.id);
				};
				...
				var copy_React = __webpack_require__(832);
				function _extends() {
				...
				var dropper_React = __webpack_require__(832);
				...
				var overflowMenu_React = __webpack_require__(832);
				...
				var tick_React = __webpack_require__(832);
				...
				var ChannelDetails_React = __webpack_require__(832);
				...
				var _patchChannelItem = new WeakSet;
				class ChannelDetails extends(external_BasePlugin_default()) {
					constructor(...args) {
						super(...args);
						_patchChannelItem.add(this);
					}
					onStart() {
						_classPrivateMethodGet(this, _patchChannelItem, _patchChannelItem2).call(this);
				...
				async function _patchChannelItem2() {
					const ChannelItem = external_PluginApi_namespaceObject.WebpackModules.getModule((m => "ChannelItem" === m?.default?.displayName));
					external_PluginApi_namespaceObject.Patcher.after(ChannelItem, "default", ((_, [props], returnValue) => ChannelDetails_React.createElement(components_namespaceObject.Tooltip, {
						spacing: 20,
						delay: 500,
						tooltipClassName: tooltip.Z.tooltip,
						text: 0 === Object.keys(props.channel.permissionOverwrites).length || 0 === Object.values(props.channel.permissionOverwrites).filter((perm => hasPermissionOverride(perm, "VIEW_CHANNEL"))).length ? null : ChannelDetails_React.createElement(ChannelTooltip, {
							overrides: getPermissionOverrides(props.channel),
							guild: stores_namespaceObject.Guilds.getGuild(props.channel.guild_id)
						}),
						position: "right"
					}, (props => {
						try {
							returnValue.props.children.props.onMouseEnter = props.onMouseEnter;
							returnValue.props.children.props.onMouseLeave = props.onMouseLeave;
						} catch (error) {
							external_PluginApi_namespaceObject.Logger.stacktrace(`Failed to assign tooltip props:`, error);
						}
						return returnValue;
					}))));
				}
